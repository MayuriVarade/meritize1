module VotesHelper
  ...
  def vote_count
    ...
    if @vote.voteable.department == @vote.voter.department 
       ...
       if @vote_other.empty?
        ...
        @update_count = VoteOtherCount.update(@voteable.id,:voteable_id => @receiver ,:start_cycle =>@voteable.start_cycle,:end_cycle => @voteable.end_cycle,:vote_count => @count,:user_id => current_user.admin_user.id)  
       end
    end 	
  end
  def update_vote_count
    ...
    if @voterdept == @voteabledept
     ...
     @update_count = @voteable.update_attributes(:voteable_id => @count.voteable_id ,:start_cycle =>@count.cycle_start_date,:end_cycle => @count.cycle_end_date,:vote_count => @voteable.vote_count - 1,:user_id => current_user.admin_user.id) 
    else
     @count = Vote.find_by_cycle_start_date_and_cycle_end_date_and_voter_id(@vote_setting1.start_cycle,@vote_setting1.end_cycle,current_user.id)
